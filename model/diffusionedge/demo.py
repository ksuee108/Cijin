...
def parse_args():
    ...
    parser.add_argument("--pre_weight", help='path of pretrained weight', type=str, default="./bsds.pt")
    parser.add_argument("--sampling_timesteps", help='sampling timesteps', type=int, default=4)
...
class Sampler(object):
    def __init__(
        ...
        data = torch.load(cfg.sampler.ckpt_path, map_location=lambda storage, loc: storage, weights_only=False)
    ...
    def slide_sample(self, inputs, crop_size, stride, mask=None, bs=8):
        ...
        If h_img < h_crop or w_img < w_crop, the input will be padded
        (reflect padding) up to at least crop_size before sliding-window
        cropping, then the output will be cropped back to the original size.
        ...
        batch_size, _, h_img_ori, w_img_ori = inputs.size()

        # --- pad inputs (and mask) up to at least crop_size ---
        pad_h = max(h_crop - h_img_ori, 0)
        pad_w = max(w_crop - w_img_ori, 0)
        ...
        if pad_h > 0 or pad_w > 0:
            seg_logits = seg_logits[:, :, :h_img_ori, :w_img_ori]
